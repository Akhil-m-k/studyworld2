...
f= cgi.FieldStorage()
pid = f.getvalue("id")
q = """ select staffName,dob,email,mobile,password from staff where id='%s'"""%(pid)
cur.execute(q)
r = cur.fetchone()
print("""
<!DOCTYPE html>
...
if prsub != None:
    ...
    q =""" UPDATE staff SET staffName='%s',dob='%s',email='%s',mobile='%s',password='%s'  WHERE id='%s' """%(prstaffname, prsdob, premail,prmob,prpass,pid)
    cur.execute(q)
    con.commit()
    print("""
        <script>
