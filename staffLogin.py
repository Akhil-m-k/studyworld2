    ...
    <section class="login">
        <div class="container">
            <div class="row">
                ...
                <h1>Login</h1>
                <label for="staff-id">Enter Id</label><br>
                <input type="text" placeholder="enter your id" name="staff-id" id="staff-id"><br>
                ...
                <a href="forgetpass.py"> forget password ?</a>
            </form>
                </div>
            </div>
        </div>
...
if psub != None:
    ...
    cur = conn.cursor()
    q = """ select id,staffId,password from staff where staffId='%s' and password='%s'""" % (pstaff_id, ppass)
    cur.execute(q)
    f = cur.fetchone()
    ...
    if f != None:
        print("""
            <script>
                location.href="staffDashboard.py?id=%s";
            </script>"""%(f[0]))
    else:
        print("""
        <script>
