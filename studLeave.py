...
f = cgi.FieldStorage()
pid = f.getvalue("id")
q = """ select studName,studId from stud where id='%s'"""%(pid)
cur.execute(q)
r = cur.fetchone()
print("""
    <!DOCTYPE html>
            ...
            <div class="row">
                <div class="col-12 d-flex justify-content-center">
                    <form action="" method="post"enctype="multipart/form-data">
                        ...
                        <input type="text" id="studname" name="studname" value="%s" placeholder="Enter staff name" required ><br>
                        <label for="studid">student id</label><br>
                        <input type="text" id="studid" name="studid" placeholder="" readonly value="%s"><br>
...
if psub != None:
 ...
 q = """ insert into studLeave(studName,studId,fromDate,toDate,totalLeave,reason,status)values('%s','%s','%s','%s','%s','%s','%s')""" % (
pstudname, pstudid, pfrdate, ptoDate, ptleave, preason,"New")
cur.execute(q)
con.commit()
print("""
            <script>
                location.href="studstudExistingLeave.py?id=%s"
